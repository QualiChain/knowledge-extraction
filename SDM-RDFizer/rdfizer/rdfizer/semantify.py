...
def semantify_xml(triples_map, triples_map_list, output_file_descriptor, csv_file, dataset_name):
	...
	with open(str(triples_map.data_source), "r", encoding="utf-8") as input_file_descriptor:
										...
										if triples_map_element.data_source != triples_map.data_source:
											if triples_map_element.triples_map_id not in join_table:
												if str(triples_map_element.file_format).lower() == "csv" or triples_map_element.file_format == "JSONPath":
													with open(str(triples_map_element.data_source), "r", encoding="utf-8") as input_file_descriptor:
...
def semantify(config_path):
	...
	if config["datasets"]["all_in_one_file"] == "no":
		...
		with open(config["datasets"]["output_folder"] + "/" +  config["datasets"]["name"] + "_datasets_stats.csv", 'w', encoding="utf-8") as myfile:
		...
		with open(config["datasets"]["output_folder"] + "/stats.csv", 'w', encoding="utf-8") as myfile:
			wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
		...
		start_time = time.time()
		with open(output_file, "w", encoding="utf-8") as output_file_descriptor:
			with open(config["datasets"]["output_folder"] + "/" + "datasets_stats.csv", 'w', encoding="utf-8") as myfile:
				wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
				wr.writerow(["Dataset", "Number of the triple", "Time"])
				with ThreadPoolExecutor(max_workers=10) as executor:
					for dataset_number in range(int(config["datasets"]["number_of_datasets"])):
